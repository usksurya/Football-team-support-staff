...
class AXValueSource:
    ...
    def from_json(cls, json: T_JSON_DICT) -> AXValueSource:
        return cls(
            ...
            invalid=(
                bool(json["invalid"])
                if json.get("invalid", None) is not None
                else None
            ),
            invalid_reason=(
...
class AXRelatedNode:
    ...
    def from_json(cls, json: T_JSON_DICT) -> AXRelatedNode:
        return cls(
            backend_dom_node_id=dom.BackendNodeId.from_json(
                json["backendDOMNodeId"]
            ),
            idref=(
                str(json["idref"])
                if json.get("idref", None) is not None
                else None
            ),
            text=(
                str(json["text"])
...
class AXNode:
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        if self.ignored_reasons is not None:
            json["ignoredReasons"] = [
                i.to_json() for i in self.ignored_reasons
            ]
...
def enable() -> typing.Generator[T_JSON_DICT, T_JSON_DICT, None]:
    ...
    to remain consistent between method calls.
    This turns on accessibility for the page,
...
def query_ax_tree(
    ...
    including those that are ignored for accessibility,
    and returns those that match the specified name and role.
    If no DOM node is specified, or the DOM node does not exist,
